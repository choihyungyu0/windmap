# backend — 데이터 파이프라인·확산·검증 (Python)
...
export.py     front/public/data/pipeline-status.json (관제 대시보드 소비)
```

## 실행 (windmap 루트에서)

```bash
python -m backend.pipeline --mock                 # 1사이클 (키 불필요)
python -m backend.pipeline --mock --backfill 72   # 과거 72시간 채우기
python -m backend.scheduler --mock --interval 60  # 데모: 60초 주기 상시 수집
python -m backend.scheduler                       # 운영: 30분 주기 (키 필요)
```

## live 전환 (P2b — 서비스 키 발급 후)

1. `front/.env.local` 에 `DATA_GO_KR_SERVICE_KEY=` 채우기
